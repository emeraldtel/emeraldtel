...
import org.devsonics.activities.main.dialer.NumpadDigitListener
import org.devsonics.activities.main.viewmodels.LogsUploadViewModel
        ...
        return if (addressToCall.isNotEmpty()) {
            onMessageToNotifyEvent.value = Event(
                if (coreContext.transferCallTo(addressToCall)) {
                    org.devsonics.R.string.dialer_transfer_succeded
                } else {
                    org.devsonics.R.string.dialer_transfer_failed
                }
            )
            eraseAll()
            true
