...
import org.devsonics.contact.ContactDataInterface
import org.devsonics.contact.NativeContactEditor
import org.devsonics.utils.AppUtils
import org.devsonics.utils.ImageUtils
...
class ContactEditorData(val friend: Friend?) : ContactDataInterface {
    ...
    fun addEmptySipAddress() {
        ...
        list.add(NumberOrAddressEditorData("", true, this))
        addresses.value = list
        sipAddressesCount++
        println("value of the su" + addresses.value!!.size + " " + numbers.value!!.size)
    }

    fun getAddressSize(): Boolean {
        println("value of the su" + addresses.value!!.size)
        return sipAddressesCount> 0
    }

    fun getNumberSize(): Boolean {
        return phoneNumberCount> 0
    }
    fun addEmptyPhoneNumber() {
        val list = arrayListOf<NumberOrAddressEditorData>()
        list.addAll(numbers.value.orEmpty())
        list.add(NumberOrAddressEditorData("", false, this))
        numbers.value = list
        phoneNumberCount++
    }
    ...
    private fun updateNumbersAndAddresses(contactId: String?) {
            ...
            try {
                ...
                while (cursor != null && cursor.moveToNext()) {
                    ...
                    if (mime == ContactsContract.CommonDataKinds.Phone.CONTENT_ITEM_TYPE) {
                        ...
                        if (data1 != null) {
                            phoneNumbers.add(NumberOrAddressEditorData(data1, false, this))
                        }
                    } else if (
                        ...
                        if (data1 != null) {
                            sipAddresses.add(NumberOrAddressEditorData(data1, true, this))
                        }
                    }
                }
        ...
        if (!fetched) {
            ...
            for (number in friend?.phoneNumbers.orEmpty()) {
                phoneNumbers.add(NumberOrAddressEditorData(number, false, this))
            }

            for (address in friend?.addresses.orEmpty()) {
                sipAddresses.add(NumberOrAddressEditorData(address.asStringUriOnly(), true, this))
            }
        }

        if (phoneNumbers.isEmpty()) {
            phoneNumbers.add(NumberOrAddressEditorData("", false, this))
        }
        numbers.value = phoneNumbers

        if (sipAddresses.isEmpty()) {
            sipAddresses.add(NumberOrAddressEditorData("", true, this))
        }
